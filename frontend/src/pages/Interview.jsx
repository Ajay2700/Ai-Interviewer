      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 text-center shadow-sm backdrop-blur-sm dark:border-slate-800 dark:bg-slate-950/70">
          ...
          <p className="mt-3 text-xs font-medium text-slate-500 dark:text-slate-400">
            You have {usageSummary?.questions_left_today ?? '--'} questions left today and{' '}
            {usageSummary?.tokens_left_today ?? '--'} tokens left.
          </p>
          <div className="mt-5 flex items-center justify-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-primary-500 to-indigo-500 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
            >
              {startingInterview && (
          ...
          {startDisabledReason && (
            <p className="mt-3 text-sm text-amber-500">{startDisabledReason}</p>
          )}
        </div>
      </div>
    );
    ...
    <div className="relative space-y-5">
      ...
      <div className="grid gap-5">
        <section className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 pr-6 shadow-sm backdrop-blur-sm dark:border-slate-800 dark:bg-slate-950/70 md:pr-64">
          ...
          <p className="mb-3 inline-flex rounded-full border border-slate-300/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-slate-600 dark:border-slate-700 dark:text-slate-300">
            {questionSource === 'database' ? 'Company Question' : 'AI Question'}
          </p>
          ...
          <div className="mt-6 flex flex-col items-center gap-4">
            ...
            >
              Skip disabled (mandatory answer)
            </button>
